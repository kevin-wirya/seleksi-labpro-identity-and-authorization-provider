...
function LogOutIcon() {
    return (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 003 H6a3 3 0 003-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
        </svg>
    );
}
...
function LogInIcon() {
    return (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 16l4-4m0 0l-4-4m4 4H3m6 4v1a3 3 0 003 H6a3 3 0 003-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
        </svg>
    );
}
...
export default async function Home() {
    ...
    if (sessionToken) {
        ...
        if (localSession) {
            ...
            try {
                ...
                const appId = appRecord ? appRecord.id : 'app-b';
                activityLogs = await prisma.auditLog.findMany({
                    where: {
                        OR: [
                            { application_id: 'app-b' },
                            { application_id: appId },
                            { metadata: { contains: 'app-b' } },
                        ],
                    },
                    take: 10,
    ...
    if (userProfile && localSession) {
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-black text-zinc-100 p-6">
                <div className="max-w-4xl w-full bg-zinc-900/80 rounded-2xl p-8 shadow-2xl border border-zinc-800 space-y-6">
                        ...
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                <div className="flex items-center space-x-2">
                                    <h1 className="text-2xl font-black text-white tracking-wide">APP B</h1>
                                    <span className="px-2 py-0.5 text-xs font-semibold bg-purple-500/10 text-purple-400 border border-purple-500/20 rounded-full">Relying Application</span>
                                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div className="bg-zinc-950/60 p-5 rounded-xl border border-zinc-800/80 space-y-3">
                            <div className="flex items-center space-x-2 text-purple-400 font-bold border-b border-zinc-800 pb-2">
                                <UserIcon />
                    ...
                    <div className="space-y-3 pt-4 border-t border-zinc-800/80">
                        <div className="flex items-center space-x-2">
                            <ActivityIcon />
                            <h3 className="font-bold text-zinc-200 text-sm tracking-wide uppercase">Activity Log</h3>
                        </div>
                                    ...
                                    <tbody className="divide-y divide-zinc-800/80 bg-zinc-900/40">
                                        {activityLogs.map((log) => (
                                            <tr key={log.id} className="hover:bg-zinc-800/40 transition-colors">
                                                ...
                                                <td className="p-3">
                                                    <span className={`px-2 py-0.5 text-xs rounded border uppercase font-bold ${log.result === 'success' || log.result === 'granted' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
                                                        {log.result}
                                                    </span>
                                                </td>
                                                <td className="p-3 font-mono text-xs text-zinc-400 max-w-xs truncate">{log.metadata || '-'}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-black text-zinc-100 p-6">
            <div className="max-w-3xl w-full bg-zinc-900/80 rounded-2xl p-8 shadow-2xl border border-zinc-800 text-center space-y-6">
                <div className="mx-auto w-16 h-16 bg-purple-500/10 border border-purple-500/20 rounded-2xl flex items-center justify-center">
                    <ShieldIcon />
                </div>
                <div>
                    <h1 className="text-3xl font-black text-white tracking-wide">APP B</h1>
                    <p className="text-zinc-400 text-sm mt-1">Relying Application B • SSO Client</p>
                </div>
                <Link href="/api/auth/login" className="flex items-center justify-center space-x-2 w-full py-3.5 bg-purple-600 hover:bg-purple-500 rounded-xl text-white font-bold transition-all shadow-lg shadow-purple-950">
                    <LogInIcon />
